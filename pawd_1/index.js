...
app.get("/", (req, res) => {
    res.json("SERVER for PAWD_1 is running");
});

let mqtt = require("mqtt");

let id = "5f956791606f0f20857ea06d";
// const options = {
...
client.on("connect", function () {
    console.log("PAWD 1 MQTT connected");
...
client.on("message", function (topic, message) {
    if (topic === `PAWD/Inactive/Open/${id}`) {
        console.log("PAWD/Inactive/Open Opening door");
...
app.listen(port, () => {
    console.log(`SERVER for PAWD_1 is running on port ${port}!`);
});
